...
class fit():
    ...
    def load_stan_model(self):
        ...
        """
        try:
            self.model = pickle.load(open('COSImodfit.pkl', 'rb')) #read COSImodefit.pkl (if already compiled)
            
        except:
            self.model = pystan.StanModel('COSImodfit.stan') # compile model (if not yet compiled)

            with open('COSImodfit.pkl', 'wb') as f: # save it to the file 'filename.pkl' for later use
